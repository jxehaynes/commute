...
struct HomeView: View {
    ...
    private var shouldShowStatusSwipeHint: Bool {
        showsStatusSwipeHint && statusRevealProgress < 0.08 && !journeyRevealed
    }
            ...
            ZStack {
                ...
                GeometryReader { proxy in
                    ...
                    ZStack(alignment: .bottom) {
                        ...
                        if !journeyRevealed, holdIsEnabled, !isStatusRevealed {
                            ...
                            .simultaneousGesture(statusSwipeGesture(maxShift: maxShift))
                            .zIndex(15)
                        }
                    }
                    ...
                    .overlay(alignment: .top) {
                        if journeyRevealed, directionsRoute == nil {
                            ...
                            homeTopBar
                                ...
                                .opacity((1 - statusRevealAmount(for: statusProgress) * 0.5) * homeContentOpacity)
                                .zIndex(20)
                        }
                    }
                }
    ...
    private func homeSwipeLayer(
        ...
        .gesture(
            isStatusRevealed ? nil : statusSwipeGesture(maxShift: maxShift),
            including: .gesture
        )
        .highPriorityGesture(
            isStatusRevealed ? statusSwipeGesture(maxShift: maxShift) : nil
        )
    ...
    private func homeBottomControls(bottomPadding: CGFloat) -> some View {
        if viewModel.needsDestinationPicker(for: appState.userProfile) {
            DestinationConflictPicker(
                destinations: viewModel.destinationCandidates(for: appState.userProfile),
                accent: accentStyle,
    ...
    private var holdHint: some View {
        Text(holdIsEnabled ? "Hold anywhere for directions" : "Set up your places to get directions")
            .font(Theme.Fonts.caption)
            .foregroundStyle(Theme.Colors.textSecondary)
            .frame(maxWidth: .infinity, alignment: .leading)
    }
    ...
    private var homeGreetingContent: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            .frame(maxWidth: .infinity, alignment: .leading)
            .allowsHitTesting(false)
            .accessibilityHidden(!shouldShowStatusSwipeHint)
        }
        .padding(.horizontal, OnboardingMetrics.horizontalPadding)
        .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private func statusSwipeGesture(maxShift: CGFloat) -> some Gesture {
        DragGesture(minimumDistance: 8)
            ...
            .onEnded { value in
                ...
                let horizontal = value.translation.width
                let vertical = abs(value.translation.height)
                guard abs(horizontal) > vertical * 0.85 else {
                    statusDragTranslation = 0
                    return
                }

                let final = min(
                    max(statusRevealProgress + horizontal / maxShift, 0),
                    1
                )
                let predictedShift = value.predictedEndTranslation.width / maxShift
                let predictedFinal = min(max(statusRevealProgress + predictedShift, 0), 1)

                withAnimation(reduceMotion ? .easeInOut(duration: 0.25) : .spring(response: 0.38, dampingFraction: 0.86)) {
